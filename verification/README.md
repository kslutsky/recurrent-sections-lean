...
dependency revisions, and SHA-256 hashes of the 39 checked Lean source files,
configuration files, and verification script.

This `main` branch excludes the partial polynomial-volume formalization
kept on `research/polynomial-volume`. The current project has 34 modules in three libraries, three root imports,
...
| Build with warnings treated as errors: 3448 build jobs | [build-output.txt](build-output.txt) |
| Signatures and logical dependencies of 58 principal results | [audit-output.txt](audit-output.txt) |
| All 477 library declarations use only allowed logical axioms, including private declarations | [all-axioms-output.txt](all-axioms-output.txt) |
